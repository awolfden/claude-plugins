...
Reference for Phase 4e of the brand-demo skill. Instead of handing off to the user to manually use Claude in Chrome, automate dashboard branding directly using Claude Code's `mcp__claude-in-chrome__*` MCP tools.
...
| Scenario | Action |
|----------|--------|
| Chrome extension not running | Fall back to manual prompt (current Phase 4e behavior) |
